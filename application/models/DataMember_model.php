...
class DataMember_model extends CI_Model
{
    ...
    public function select4()
    {
        $data = $this->db->select('*')
            ->from('user as u')
            ->join('member as m', 'u.id_user=m.id_user')
            ->get()->result();
        return $data;
    }
    public function getUserById($id_user)
    {
        return $this->db->get_where('user', ['id_user' => $id_user])->row(); // asumsi tabelnya 'user'
    }
    ...
    {
        $res = $this->db->update('member', $data, array('id_user' => $id));
        return $res;
    }
